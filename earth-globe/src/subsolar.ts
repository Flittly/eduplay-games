 ...
 */
export const DATE_ANCHORS: { nu: number; day: number }[] = [
  { nu: 0, day: 4 },
  { nu: 76, day: 79 },
  { nu: 166, day: 172 },
  { nu: 256, day: 265 },
...
export const YEAR_DAYS = DATE_ANCHORS[DATE_ANCHORS.length - 1].day - DATE_ANCHORS[0].day;
 ...
 */
export function nuToDay(nuDeg: number): number {
...
export function dayToNu(day: number): number {
...
export interface SeasonMark {
  key: "chunfen" | "xiazhi" | "qiufen" | "dongzhi";
  label: string;
  /** 日期文字（与引擎 SEASONS 的 date 同源口径） */
  date: string;
  nu: number;
  decl: number;
}

const SEASON_SEEDS: { key: SeasonMark["key"]; label: string; date: string; nu: number }[] = [
  { key: "chunfen", label: "春分", date: "3月21日", nu: 76 },
  { key: "xiazhi", label: "夏至", date: "6月22日", nu: 166 },
  { key: "qiufen", label: "秋分", date: "9月23日", nu: 256 },
  { key: "dongzhi", label: "冬至", date: "12月22日", nu: 346 }
];

export const SEASON_MARKS: SeasonMark[] = SEASON_SEEDS.map((item) => {
  const raw = declinationForNu(item.nu);
  // 二分点理论上是 0：浮点残差会算出 -0.0 / -1e-16，读出来是「-0.0°」，抹掉
  return { ...item, decl: Math.abs(raw) < 1e-9 ? 0 : raw };
});

/** 当前公转位置离哪个节气最近（返回 key 与角距离，单位：度） */
export function nearestSeason(nuDeg: number): { mark: SeasonMark; distance: number } {
  ...
  return { mark: best, distance: bestD };
}
 ...
 */
export const SUBSOLAR_SVG = { w: 172, h: 118 };
/** 绘图区：左侧 32px 留白给参考线文字（23.5°N 之类），下方 16px 给节气刻度 */
export const SUBSOLAR_PLOT: ChartBox = { x: 32, y: 10, w: 128, h: 92 };
export const SUBSOLAR_TICK_FONT = 8;
export const SUBSOLAR_REF_FONT = 7.5;

/** 公转位置 → 画布 x（0° 在最左、360° 在最右） */
export function nuToX(nuDeg: number, box: ChartBox): number {
 ...
 */
export function declToY(decl: number, box: ChartBox): number {
...
export interface SubsolarChart {
  box: ChartBox;
  /** 曲线数据点（含画布坐标，画折线用） */
  points: CurvePoint[];
  path: string;
...
export function pathFromPoints(points: { x: number; y: number }[], precision = 2): string {
  if (points.length === 0) {
    return "";
  }
...
export function buildSubsolarChart(box: ChartBox, tickFont = 8): SubsolarChart {
  ...
  return {
    ...
    refLines: [
      ...
      { key: "tropicS", label: "23.5°S", decl: -EARTH_TILT, y: declToY(-EARTH_TILT, box) }
    ],
...
export function xToNu(x: number, box: ChartBox): number {
