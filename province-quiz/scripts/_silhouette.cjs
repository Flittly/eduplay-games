...
*/
const TINY_STROKE_PX = 1.5;
/** 要素 bbox 的长边（视图单位）。 */
function bboxSize(feature) {
	const [minX, minY, maxX, maxY] = feature.bbox;
	return Math.max(maxX - minX, maxY - minY);
}
/** 是不是小到「描边会糊死形状」的要素。实测只圈中香港、澳门。 */
function isTiny(feature) {
	return bboxSize(feature) < 22;
}
/**
...
*/
function silhouetteViewBox(feature) {
	const [minX, minY, maxX, maxY] = feature.bbox;
	const w = maxX - minX;
	const h = maxY - minY;
	const longest = Math.max(w, h);
	const pad = longest * .08 + (isTiny(feature) ? 0 : 12) + (longest > 0 ? 0 : 1);
...
*/
function silhouettePathClass(feature) {
	return isTiny(feature) ? "silhouette-path is-tiny" : "silhouette-path";
}
//#endregion
exports.TINY_BBOX = TINY_BBOX;
exports.TINY_STROKE_PX = TINY_STROKE_PX;
exports.bboxSize = bboxSize;
exports.isTiny = isTiny;
exports.silhouettePathClass = silhouettePathClass;
exports.silhouetteViewBox = silhouetteViewBox;
