...
## 确定性像素采样：3D 场景做像素对比的唯一可靠姿势

**软渲染下 `capturePage` 会抓到"画了一半的帧"**（同一状态两次截图差可达 28 万像素），
所以凡是"开/关某图层前后亮度/颜色有没有变"这类对比，**一律不要用截图**，改用引擎内确定性采样：
`renderer.render(scene, camera)` + `gl.readPixels` **放在同一个 JS 任务内**完成
（本项目钩子 `__globeDebug.samplePixels(pts)`）。同状态两次采样差 0~97，可当天平用。

两个必须遵守的细节：
- **采样点要半格错相** `(i+0.5)/n`。x 取地心那一列正好压着 1~2px 宽的晨昏线细线，
  两种抗锯齿结果会让"确定性"自检**假报不一致**。
- 构造「开/关图层」对比时**必须先把自转与自动旋转都关掉**（`setRotate(false)` +
  `setAutoRotate(false)`；只 `setSpin(角度)` 不停止自转是不够的），
  并断言两次采样之间地球屏幕位置漂移 < 1px，否则对比会被自转造成的明暗移位带偏而失效。

## 椭圆轨道上求极值：不能用两点线性外推

要在公转轨道上找"亮度最高/最低对应的公转位置"（如正午、直射点），
**不能用两点线性外推**——`ORBIT_E=0.16` 时残差可达 **8.4°**。
正确做法：**粗扫 24 步 × 15° + 在粗扫极值附近细扫 ±15°（步长 2.5°）**，取盘心亮度极值。

且**搜索前必须先关掉无关图层**：找"正午"时忘了关城市灯光，
上海午夜灯火（亮度 249）比白天（171）还亮，直接把最亮点定到了午夜。
**本机亮度标定**：夜 40 / 昼 171 / 夜+灯火 249 ⇒ 判"白天"用 `>130`，不是 `>200`。
